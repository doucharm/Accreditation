...
def generate_order(student_id,semester_id,classificationtype_id):
    print('here',classificationtype_id)
    rand_num = random.randint(0, 99)
    if rand_num < 90:  # 90% probability
        return [
            new_classification(student_id,semester_id,classificationtype_id,order=1)
        ]
    elif rand_num < 99:  # 9% probability
        return [
            new_classification(student_id,semester_id,classificationtype_id,order=1),
            new_classification(student_id,semester_id,classificationtype_id,order=2)
        ]
    else:  # 1% probability 
        return [
            new_classification(student_id,semester_id,classificationtype_id,order=1),
            new_classification(student_id,semester_id,classificationtype_id,order=2),
            new_classification(student_id,semester_id,classificationtype_id,order=3)
        ]
def new_classification(student_id,semester_id,classificationtype_id,order):
    print(classificationtype_id,'printee')
    new_classification = {
...
for group in data['groups']:
    if group['grouptype_id'] == "cd49e157-610c-11ed-9312-001a7dda7110": #check if a group is study group 
        ...
        for program in data['acprograms_students']:
            if program['student_id'] == students_id[0]:
            ...
            if subject['program_id'] == program_id:
                for semester in data['acsemester']:
                    if semester['subject_id'] == subject['id']:
                        ...
                        print(semester_id,"+",classificationtype_id)
                        for student in students_id:
                            generated_classification.extend(generate_order(semester_id=semester_id, classificationtype_id=classificationtype_id, student_id=student))
